...
export default function Input() {
  ...
  return (
    <>
      {session && (
        <div className="flex border-b border-gray-400 p-3 space-x-3 ">
          <img 
              onClick={signOut}
              src={session.user.image} 
              alt="user-img" 
              className="h-11 w-11 rounded-full cursor-pointer xl:mr-2 hover:brightness-90"
          />
          <div className="w-full divide-y divide-gray-400">
              ...
              {selectedFile && (
                <div className="relative">
                  <XIcon className="border h-7 absolute cursor-pointer shadow-md border-white m-1.5 rounded-full" onClick={()=> setSelectedFile(null)}/>
                  <img src={selectedFile} alt="selectedFile" className={`${loading && "animate-pulse"}`} />
                </div>
              )}
